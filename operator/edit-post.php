...
$postToEdit = new Post(); //Instantiate post class
...
if(filter_input(INPUT_POST, "submit")!==NULL){
    ...
    if(count($errorArr) < 1)   {$postObj->id = $_SESSION['postToEdit']; if($postObj->update($dbObj)==='success') {$msg = $thisPage->messageBox('Post successfully updated. Go back to <a href="posts">all posts</a>', 'success');} else {$msg = $thisPage->messageBox('Post update failed.', 'error');}}
	    ...
	    <div class="row">
			...
			<div class="widget-content">
                            <div class="content clearfix">
                                ...
                                if(isset($_SESSION['postToEdit'])){
                                    $postVar = $_SESSION['postToEdit'];
                                    $posToEditDets = $postToEdit->fetch($dbObj, $column="*", $condition=" id = $postVar");
                                    foreach($posToEditDets as $posToEditDet){
                                ?>
                                <fieldset>						
                                    <div class="control-group">											
                                        <label class="control-label" for="title">Post Title</label>
                                        <div class="controls">
                                            <input type="text" class="span7 input-large " id="title" name="title" value="<?php echo $posToEditDet['title']; ?>" placeholder="Enter post title">
                                        </div> <!-- /controls -->				
                                    </div> <!-- /control-group -->
                                    ...
                                    <div class="control-group">											
                                        ...
                                        <div class="controls">
                                            <textarea name="content" placeholder="enter post content" id="content" class="span7"><?php echo $posToEditDet['content']; ?></textarea>
                                            <script type="text/javascript">
                                    ...
                                    <div class="control-group">											
                                        <label class="control-label" for="metaDescription">Meta Description</label>
                                         <div class="controls">
                                            <textarea name="metaDescription" placeholder="Enter meta description" id="metaDescription" class="span7"><?php echo $posToEditDet['meta_description']; ?></textarea>
                                        </div>	<!-- /controls -->			
                                    </div> <!-- /control-group -->
                                    <div class="control-group">											
                                       
                                        <div class="controls">
                                            <input type="checkbox" <?php if($posToEditDet['status']==1) {echo "checked='checked'";} ?> id="status" name="status" value="1"> <label for="status"> Activation Status</label>
                                        </div> <!-- /controls -->				
                                    </div> <!-- /control-group -->
                                    <input type="hidden" name="featuredImage" value="<?php echo $posToEditDet['featured_image'];?>" id="featuredImage">
			...
			<div class="widget-content">
                            <div class="content clearfix">
                            <?php
                                $fetPostCatIds = explode(':', $posToEditDet['category']); 
                                ...
                                if(count($categories)> 0){ 
                                    ...
                                    foreach($categories as $categories){$checkedAttr = '';
                                        ...
                                        if($fetPostCatIds[$i]==$categories['id']){$checkedAttr = 'checked="checked"';}
                                        }
                                       $list .= '<li style="list-style-type: none;"><input type="checkbox" '.$checkedAttr.' id="category'.$categories['id'].'" name="category[]" value="'.$categories['id'].'"> <label for="category'.$categories['id'].'">'.$categories['name'].'</label></li>';    
                                    }
                                    $list .= '</ul>';
                                }
                                echo $list;
			...
			<div class="widget-content">
                            <div class="content clearfix">
                                <div class="image-full col-md-12" style="height:150px;" id="featured">
                                    <!-- image here -->
                                    <img src="<?php echo $posToEditDet['featured_image']; }}?>" style="max-height: 150px;" width="330" alt=""/>
                                </div>
                                <div>
